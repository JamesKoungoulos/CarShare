...
def list_bays():
    ...
    if (request.method == 'GET'):
        ...
        if(val is None):
            ...
            page['bar'] = False
        cars = database.get_cars_in_bay(val[5])
        if(cars is None):
            cars = []
        ...
        # If no matching bays were found
        if(val is None or len(val) < 1):
            flash("No results found.")
            page['bar'] = False
            val = []
            val.append([' ','No results found',' '])
...
def new_booking():
    ...
    if(success == 0):
        user_details['num_bookings'] += 1 #we need to reflect the successful booking in the session as well as database
        page['bar'] = True
        flash("Booking Successful!")
        return(redirect(url_for('my_bookings')))
    elif(success == 1):
        page['bar'] = False
        flash("Car already booked for this period!")
        return(redirect(url_for('new_booking')))
    elif(success == 2):
        page['bar'] = False
        flash("You already have a booking for this period!")
        return(redirect(url_for('new_booking')))
    elif(success == 3):
        page['bar'] = False
        flash("Please enter an appropriate time.")
        return(redirect(url_for('new_booking')))
    else:
        page['bar'] = False
